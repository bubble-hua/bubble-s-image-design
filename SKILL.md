---
name: bubble-design-system
description: 花心小泡泡个人IP设计系统模板。做HTML页面、个人网站、教程页面、介绍页面、landing page等任何前端设计时自动触发。包含品牌DNA和多个场景子规范。
---

> ❗ **首次使用前必须完成配置：**
> 1. 品牌色已配置为花心小泡泡的棕/黄/蓝三色。如需调整，修改 `brand-dna.md` 中的三色色值，并同步修改 `assets/template-*.html` 里 `:root` 的 CSS 变量（三色比例原则不变：主色60% / 强调色30% / 点缀色10%）
